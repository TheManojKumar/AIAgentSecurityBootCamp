# Week 2 — Command & Concept Reference · INSTRUCTOR EDITION
...
>
> **This is the instructor edition.** It includes the prep only you run (validate the lab locally), the `team_hardened.py` run, delivery cues, and margin notes. A separate **Attendee edition** (`Commands Reference - Attendee.md`) omits all of that — hand that one to students.
>
...
>
> **The one thing students must leave with:** individually safe agents can compose into an unsafe system. An orchestrator that trusts a subagent's output implicitly will execute actions it would never take on its own. Trust between agents is a boundary that must be *designed*, not assumed.
...
```bash
# 0.1 — Ollama serving + models present (Week 2 now uses the specialist)
ollama list
...
## Section 1 — BUILD: a supervisor + two specialists (~35 min)
...
docker compose run --rm agent python team.py "Summarize what the corpus says about wind power."
```
Researcher fetches `wind.txt`, writer summarizes, clean output.

> **Delivery cue:** open Phoenix (`http://localhost:6006`) and show the three-node span tree — supervisor → researcher → writer.
>
> **Say:** "Watch the boundary between researcher and supervisor. Right now the supervisor reads research content with full trust."

---

## Section 2 — ATTACK: indirect injection → cascade (~40 min)

The corpus already contains a poisoned `corpus/solar.txt` ending with hidden instructions.

**Show the poison (`attacks/poisoned_corpus_note.txt` — the note appended to `solar.txt`):**
```
...
> **Say:** "This is indirect injection. The attacker never spoke to the agent — they planted text in a document. Notice the *cascade*: one poisoned hop (ASI08) propagated through an unguarded inter-agent boundary (ASI07) all the way to the user."

**[STRETCH — Tier A, discussion moment]** Add a third specialist that also reads the supervisor's now-corrupted output, showing the blast radius grow with each hop.

---

## Section 3 — DEFEND: guard the boundaries (~45 min)

Apply each layer, then **re-run the poisoned solar request** so students see each control's contribution.
...
### The full hardened team (all four layers — instructor copy, not distributed to students)
```bash
docker compose run --rm agent python team_hardened.py "Summarize what the corpus says about solar power."
```
...
> **The landing line:** every place one agent reads another's output is a trust boundary. We defended it four ways — reframe content as data, screen it with a validator, constrain it with a schema, scope tools so a breach can't escalate. That's the multi-agent version of defense in depth.
...
├── team_hardened.py             # all four layers — INSTRUCTOR ONLY, omit from student distribution
└── README.md
```
**Why a mock corpus instead of live web:** keeps the lab fully offline and the attack 100% reproducible on screen. The corpus file *is* the "indirect" channel — same threat model as a poisoned web page, without network access.
...
## Instructor margin notes — Week 2

- **Most common misconception:** "the validator node solves it." Show in exercise 2 that a pattern/keyword validator misses *semantic* manipulation — that motivates the schema and data-framing layers.
- **Demo reliability:** small models sometimes ignore the injected note by luck. Run the attack 2–3 times, or use `llama3.2:1b` as the specialist so the cascade reliably fires on screen.
